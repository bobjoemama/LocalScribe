...
describe("shortcut recorder keyboard capture", () => {
  ...
  it("enables capture with macOS shortcut labels", () => {
    const ready = renderToStaticMarkup(createElement(ShortcutRecorder, {
      kind: "toggle",
    ...
    expect(ready).toContain("Command + Space");
  });
  ...
  it("uses physical key codes and keeps modifiers in accelerator order", () => {
    ...
    expect(shortcutFromKeyboardEvent(keyEvent("MetaLeft", { metaKey: true }))).toBe("Command");
  });
  ...
  it("keeps a bare modifier available for hold-to-talk validation", () => {
    ...
    expect(shortcutFromKeyboardEvent(keyEvent("NumpadEnter"))).toBe("NumpadEnter");
  });
});
